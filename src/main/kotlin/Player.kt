...
class Player(private val nw: Network, private val predicateMoves: Int = 2) {
    /**
    ...
    fun selectMove(checkerboard: Checkerboard, color: Int, steps: List<String>): String {
        val list = steps.map { it to play(checkerboard, color, predicateMoves, it) }
        val max = list.maxBy { it.second }!!.second
        val l = list.filter { it.second == max }
        return l[round(random()*(l.size-1)).toInt()].first
    }
    ...
    private fun selectBestStep(checkerboard: Checkerboard, color: Int, steps: List<String>): String {
        ...
        val list = steps.map { it to GameController(checkerboard.clone()) }.map { (command, game) ->
            ...
            command to o[0]
        }.map { it.first to it.second * (1.1 - random() / 5) } // закладываем ошибку +/- 10%
        return (if (color == 0) {
